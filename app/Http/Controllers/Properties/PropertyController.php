...
class PropertyController extends Controller
{
    ...
    public function search(Request $request){
        ...
        Session::forget('search_names');
        Session::forget('search_inputs');

        // add field names to session arrays for search result summary
        if(!empty($input['term'])) {
            $_SESSION['search_names']['term'] = $input['term'];
        }
        if(!empty($input['property_type_id'])){
            $_SESSION['search_names']['property_type_id'] = PropertyType::getName($input['property_type_id']);
        }
        if(!empty($input['state_id'])){
            $_SESSION['search_names']['state_id'] = State::getName($input['state_id']);
        }
        if(!empty($input['bedrooms'])){
            $_SESSION['search_names']['bedrooms'] = $input['bedrooms'];
        }
        if(!empty($input['living_rooms'])){
            $_SESSION['search_names']['living_rooms'] = $input['living_rooms'];
        }
        if(!empty($input['max_cost'])){
            $_SESSION['search_names']['max_cost'] = $input['max_cost'];
        }
        if(!empty($input['min_cost'])){
            $_SESSION['search_names']['max_cost'] = $input['max_cost'];
        }

        // Add array sessions to a new session called search_values to bee seen by the export method
        Session::put('search_values', $_SESSION['search_names']);
        Session::put('search_inputs', $input);
        ...
        $data['properties'] = PropertyDetail::with('state', 'property_type', 'realtor')
            ->wherehas('property_type', function ($query) use ($input){
                ...
                $query->when(!empty($input['term']), static function($term) use($input){
                    ...
                    return $term->where('property_type_id', 'like' , '%'. $input['property_type_id'] .'%');

                })->when(!empty($input['state_id']), static function ($term) use($input){
                    return $term->where('state_id', 'like' , '%'. $input['state_id'] .'%');

                })->when(!empty($input['bedrooms']), static function ($term) use($input){
                    return $term->where('bedrooms', 'like' , '%'. $input['bedrooms'] .'%');

                })->when(!empty($input['bathrooms']), static function ($term) use($input){
                    return $term->where('bathrooms', 'like' , '%'. $input['bathrooms'] .'%');

                })->when(!empty($input['living_rooms']), static function ($term) use($input){
                    return $term->where('living_rooms', 'like' , '%'. $input['living_rooms'] .'%');
